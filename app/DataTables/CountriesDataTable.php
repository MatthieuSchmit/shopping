    ...
    public function dataTable($query) {
        return datatables()
            ...
            ->addColumn('edit', function ($country) {
                return '<a href="' . route('countries.edit', $country->id) . '" class="btn btn-xs btn-warning btn-block">Modifier</a>';
            })
            ->addColumn('destroy', function ($country) {
                return '<a href="' . route('countries.destroy.alert', $country->id) . '" class="btn btn-xs btn-danger btn-block ' . ($country->ranges->count() || $country->addresses->count() || $country->order_addresses->count() ? 'disabled' : '') .'">Supprimer</a>';
            })
