    ...
    public function dataTable($query) {
        return datatables()
            ...
            ->addColumn('edit', function ($state) {
                return '<a href="' . route('states.edit', $state->id) . '" class="btn btn-xs btn-warning btn-block">Modifier</a>';
            })
            ->addColumn('destroy', function ($state) {
                return '<a href="' . route('states.destroy.alert', $state->id) . '" class="btn btn-xs btn-danger btn-block ' . ($state->orders->count() ? 'disabled' : '') .'">Supprimer</a>';
            })
