                ...
                <table border="0" cellpadding="0" cellspacing="0" role="presentation" style="vertical-align:top;" width="100%">
                  <tr>
                    <td align="center" style="font-size:0px;padding:10px 25px;padding-top:28px;padding-right:25px;padding-left:25px;word-break:break-word;">
                      <div style="font-family:Helvetica;font-size:20px;line-height:1;text-align:center;color:#FFFFFF;">Commande référence <strong>{{ $order->reference }}</strong></div>
                    </td>
                  </tr>
                  <tr>
                    <td align="center" style="font-size:0px;padding:10px 25px;padding-right:25px;padding-left:25px;word-break:break-word;">
                      <div style="font-family:Helvetica;font-size:20px;line-height:1;text-align:center;color:#FFFFFF;">Bonjour {{ $user->firstname . ' ' . $user->name }}</div>
                    </td>
                  </tr>
                  <tr>
                    <td align="center" style="font-size:0px;padding:10px 25px;padding-right:25px;padding-left:25px;word-break:break-word;">
      ...
      <![endif]-->
                  ...
                  <tr>
                    <td align="left" style="font-size:0px;padding:10px 25px;padding-right:25px;padding-bottom:20px;padding-left:25px;word-break:break-word;">
                      <div style="font-family:Helvetica;font-size:16px;line-height:1;text-align:left;color:#FFFFFF;"><br /> Date : {{ $order->created_at->format('d/m/Y') }}<br /><br /> Paiement : {{ $order->payment_text }}</div>
                    </td>
                  </tr>
                </table>
              </div>
          ...
          <![endif]-->
              <div class="mj-column-per-50 mj-outlook-group-fix" style="font-size:0px;text-align:left;direction:ltr;display:inline-block;vertical-align:top;width:100%;">
                <table border="0" cellpadding="0" cellspacing="0" role="presentation" style="vertical-align:top;" width="100%">
                  <tr>
                    <td align="left" style="font-size:0px;padding:10px 25px;padding-right:25px;padding-bottom:20px;padding-left:25px;word-break:break-word;">
                      <div style="font-family:Helvetica;font-size:16px;line-height:1;text-align:left;color:#FFFFFF;"><strong>Adresse de facturation @if($order->addresses->count() === 1 && !$order->pick) et de livraison @endif</strong><br /><br /> @isset($order->addresses->first()->name) {{ $order->addresses->first()->civility . ' ' . $order->addresses->first()->last_name . ' ' . $order->addresses->first()->first_name }}<br /> @endisset @if($order->addresses->first()->company) {{ $order->addresses->first()->company }}<br /> @endif {{ $order->addresses->first()->address }}<br /> @if($order->addresses->first()->address_bis) {{ $order->addresses->first()->address_bis }}<br /> @endif @if($order->addresses->first()->bp) {{ $order->addresses->first()->bp }}<br /> @endif {{ $order->addresses->first()->postal . ' ' . $order->addresses->first()->city }}<br />
                        {{ $order->addresses->first()->country->name }}<br />
                        {{ $order->addresses->first()->phone }}<br />
                      </div>
                    </td>
                  </tr>
                </table>
          ...
          <![endif]-->
              <div class="mj-column-per-50 mj-outlook-group-fix" style="font-size:0px;text-align:left;direction:ltr;display:inline-block;vertical-align:top;width:100%;">
                <table border="0" cellpadding="0" cellspacing="0" role="presentation" style="vertical-align:top;" width="100%">
                  <tr>
                    <td align="left" style="font-size:0px;padding:10px 25px;padding-right:25px;padding-bottom:20px;padding-left:25px;word-break:break-word;">
                      <div style="font-family:Helvetica;font-size:16px;line-height:1;text-align:left;color:#FFFFFF;">@if($order->addresses->count() > 1) <strong>Adresse de livraison</strong><br /><br /> @isset($order->addresses->first()->first_name) {{ $order->addresses->last()->civility . ' ' . $order->addresses->last()->last_name . ' ' . $order->addresses->last()->first_name }}<br /> @endisset @if($order->addresses->last()->company) {{ $order->adresses->last()->company }}<br /> @endif {{ $order->addresses->last()->address }}<br /> @if($order->addresses->last()->address_bis) {{ $order->addresses->last()->address_bis }}<br /> @endif @if($order->addresses->last()->bp) {{ $order->addresses->last()->bp }}<br /> @endif {{ $order->addresses->last()->postal . ' ' . $order->addresses->last()->city }}<br />
                        {{ $order->addresses->last()->country->name }}<br />
                        {{ $order->addresses->last()->phone }}<br /> @endif
                      </div>
                    </td>
                  </tr>
                </table>
                  ...
                  <tr>
                    <td align="left" style="font-size:0px;padding:10px 25px;padding-top:10px;padding-right:25px;padding-left:25px;word-break:break-word;">
                      <div style="font-family:Helvetica;font-size:16px;line-height:1;text-align:left;color:#FFFFFF;">@if($order->payment === 'cheque') <p>Veuillez nous envoyer un chèque avec :</p>
                        ...
                        </ul> @if($order->pick) <p><strong>Vous pourrez venir chercher votre commande dès réception du paiement.</strong></p> @else <p><strong>Votre commande vous sera envoyée dès réception du paiement.</strong>.</p> @endif Pour toute question ou information complémentaire merci de contacter notre <a href="mailto:{{ $shop->email }}">support client</a>. @endif @if($order->payment === 'mandat') <p>Vous avez choisi de payer par mandat administratif. Ce type de paiement est réservé aux administrations.</p>
                        ...
                        <ul>
                          ...
                          <li>n'oubliez pas d'indiquer votre référence de commande <strong>{{ $order->reference }}</strong></li>
                        </ul> @if($order->pick) <p><strong>Vous pourrez venir chercher votre commande dès réception du paiement.</strong></p> @else <p><strong>Votre commande vous sera envoyée dès réception du paiement.</strong>.</p> @endif Pour toute question ou information complémentaire merci de contacter notre <a href="mailto:{{ $shop->email }}">support client</a>. @endif @if($order->payment === 'carte') <p>Vous avez choisi de payer par carte bancaire. Nous vous transmettrons un email de confirmation dès que votre réglement aura été effectué.</p> @if($order->pick) <p><strong>Vous pourrez venir chercher votre commande dès réception du paiement.</strong></p> @else <p><strong>Votre commande vous sera envoyée dès réception du paiement.</strong>.</p> @endif Pour toute question ou information complémentaire merci de contacter notre <a href="mailto:{{ $shop->email }}">support client</a>. @endif
                      </div>
                    </td>
                  </tr>
                </table>
